jQuery(document).ready(function () {
  jQuery(document).on("click", ".um-popup-overlay", function () {
    ...
    jQuery(document).on(
      "click",
      'a[data-modal^="um_"], span[data-modal^="um_"], .um-modal:not(:has(.um-form)) a',
      function (e) {
      ...
      function (e) {
        ...
        var a = jQuery(this).parents(".um-modal-body"),
          e = jQuery(this)
            ...
            .attr("href"),
          t = a.find(".um-single-file-upload").data("set_mode");
        return (
          jQuery.ajax({
      ...
      function (e) {
        ...
        var a = jQuery(this).parents(".um-modal-body"),
          e = jQuery(this)
            ...
            .attr("src"),
          t = a.find(".um-single-image-upload").data("set_mode");
        return (
          jQuery.ajax({
            url: wp.ajax.settings.url,
            type: "post",
    ...
    jQuery(document).on(
      ...
      function () {
        var a,
          e,
          t = jQuery(this),
          i = jQuery(this).attr("data-key"),
          r = jQuery(this)
            .parents(".um-modal-body")
            .find(".um-single-image-preview"),
          u = r.find("img").attr("src"),
          d = r.attr("data-coord"),
          r = r.find("img").data("file"),
          m = 0,
          n =
            ...
            0),
          o = "";
        1 === jQuery('div.um-field-image[data-key="' + i + '"]').length &&
          ((n = (e = jQuery('div.um-field-image[data-key="' + i + '"]').closest(
            ".um-form"
          ))
                  ...
                  e.success &&
                    ...
                    "profile_photo" === i
                      ? jQuery(".um-profile-photo-img img").attr(
                          "src",
                          e.data.image.source_url + "?" + a.getTime()
                        )
                      : "cover_photo" === i &&
                        (jQuery(".um-cover-e")
                          .empty()
                    ...
                    jQuery(".um-single-image-preview[data-key=" + i + "]")
                      .fadeIn()
                      .find("img")
                      .attr("src", e.data.image.source_url + "?" + a.getTime()),
                    um_remove_modal(),
                    jQuery("img.cropper-invisible").remove(),
                    jQuery(".um-single-image-preview[data-key=" + i + "]")
                      .parents(".um-field")
                      .find(".um-btn-auto-width")
                      .html(t.attr("data-change")),
                    jQuery(".um-single-image-preview[data-key=" + i + "]")
                      .parents(".um-field")
                      .find('input[type="hidden"]')
                      .val(e.data.image.filename));
                },
              }))
            : ((a = new Date()),
              jQuery(".um-single-image-preview[data-key=" + i + "]")
    ...
    jQuery(document.body).on(
      ...
      function (e) {
        var a = jQuery(this).attr("data-modal"),
          t = "normal";
        jQuery(this).data("modal-size") &&
          (t = jQuery(this).data("modal-size")),
          jQuery(this).data("modal-copy") &&
            ...
            0 < jQuery('input[type="hidden"][name="user_id"]').length &&
              jQuery("#" + a).attr(
                "data-user_id",
