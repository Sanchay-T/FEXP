jQuery(function ($) {
  ...
  FreezeTable.prototype.maybe_disable = function () {
    var settings = this.get_breakpoint_options(),
      $table = this.el.$table,
      $container = $table.closest(".frzTbl").length
        ? $table.closest(".frzTbl")
        : $table.parent(),
      container_width = settings.wrapperWidth
        ? settings.wrapperWidth
        : $container.width(),
  ...
  FreezeTable.prototype.build = function () {
    ...
    var settings = this.get_breakpoint_options(),
      $table = this.el.$table,
      $temp_wrapper = $(
        '<div class="frzTbl frzTbl--temp-wrapper">'
      ).insertBefore($table),
      tpl_master = $("#frzTbl-tpl").html(),
      overflow_permit = this.get_overflow_permission();

    if (!overflow_permit && !settings.tableWidth) {
    ...
    var wrapper_width = this.options.wrapperWidth
        ? this.options.wrapperWidth
        : "",
      wrapper_height = this.options.wrapperHeight
        ? this.options.wrapperHeight
        : table_height;
    ...
    this.tpl = tpl_master
      ...
      .replace(
        /{{wrapper_width}}/g,
        wrapper_width ? "width:" + wrapper_width + "px; " : ""
      )
    ...
    this.el.$frozenColumnsInner = this.el.$frozenColumnsWrapper.children(
      ".frzTbl-frozen-columns-wrapper__inner"
    );
    this.el.$frozenColumnsLeft = this.el.$frozenColumnsInner.children(
    ...
    this.el.$frozenColumnsRight = this.el.$frozenColumnsInner.children(
  ...
  FreezeTable.prototype.grab_and_scroll = function () {
    ...
    if (this.get_breakpoint_options().grab_and_scroll) {
      ...
      $body.on(
        "mousemove.freeze_table.grab_and_scroll_" + _.namespace,
        function (e) {
  ...
  FreezeTable.prototype.build_left = function () {
    ...
    if (!settings.left) {
      ...
      this.tpl = this.tpl.replace(
        /{{hide-left-columns-heading}}/g,
        "frzTbl-fixed-heading-wrapper__columns--empty"
      );
      this.tpl = this.tpl.replace(
        /{{hide-left-column}}/g,
    ...
    this.tpl = this.tpl.replace(
      /{{left-columns-heading}}/g,
      $left_heading[0].outerHTML || ""
    );
  ...
  FreezeTable.prototype.build_right = function () {
    ...
    this.tpl = this.tpl.replace(
      /{{right-columns-heading}}/g,
      $right_heading[0].outerHTML || ""
    );
  ...
  FreezeTable.prototype.page_scroll = function (e) {
    ...
    var heading_rect =
        this.el.$fixedHeadingWrapperOuter[0].getBoundingClientRect(),
  ...
  FreezeTable.prototype.wrapper_wheel = function (e) {
    if (
      ...
      (!e.originalEvent.deltaX ||
        Math.abs(e.originalEvent.deltaY / e.originalEvent.deltaX) > 0.1) &&
    ...
    this.scroll_clear = setTimeout(function () {
    ...
    if (!this.options.captureScroll || !this.options.wrapperHeight) {
      if (
        // no scroll
        this.el.$scrollOverlay[0].scrollHeight ==
          this.el.$scrollOverlay.height() ||
        // scroll down
        (e.originalEvent.deltaY > 0 &&
          this.el.$scrollOverlay[0].scrollTop +
            this.el.$scrollOverlay.height() ==
            this.el.$scrollOverlayInner.height()) ||
        // scroll up
        (e.originalEvent.deltaY < 0 && !this.el.$scrollOverlay[0].scrollTop)
      ) {
  ...
  FreezeTable.prototype.wrapper_touch = function (e) {
    ...
    if (e.type == "touchmove" && this.ev.prevClientX !== false) {
    ...
    if (e.type == "touchend") {
      if (this.ev.animScroll) {
        this.el.$scrollOverlay.animate(
          { scrollLeft: this.ev.animScroll },
          {
            specialEasing: {
  ...
  FreezeTable.prototype.horizontal_scroll = function (deltaX, deltaY) {
    ...
    if (mode == "transform") {
      // -- table
      this.el.$tableInner[0].style.transform =
        "translate3d(-" + scrollLeft + "px, 0, 0)";
      // -- fixed heading
      this.el.$fixedHeadingInner[0].style.transform =
        "translate3d(-" + scrollLeft + "px, 0, 0)";
      // -- fixed columns
  ...
  FreezeTable.prototype._cell_resize = function (cell) {
    ...
    if (!this.get_overflow_permission() && !settings.tableWidth) {
  ...
  FreezeTable.prototype.antiscroll = function () {
    ...
    this.el.$wrapper
      .children("> .frzTbl-antiscroll-wrap")
      .remove()
      .end()
      .children(".antiscroll-scrollbar-horizontal")
      .wrap('<div class="frzTbl-antiscroll-wrap">');
  };
  ...
  FreezeTable.prototype.unwrap = function () {
    ...
    $table.removeClass("frzTbl-table").css({
      width: "",
      "min-width": "",
    });
