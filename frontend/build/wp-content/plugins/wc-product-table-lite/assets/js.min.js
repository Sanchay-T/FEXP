jQuery(function ($) {
  ...
  function get_device() {
    ...
    return (
      $(window).width() <= wcpt_params.breakpoints.phone
        ? (device = "phone")
        : $(window).width() <= wcpt_params.breakpoints.tablet &&
          (device = "tablet"),
      device
    );
  }
  function get_device_table($wcpt) {
    var device = get_device(),
      table_selector =
        ".wcpt-table-scroll-wrapper-outer.wcpt-device-laptop:visible > .wcpt-table-scroll-wrapper > .wcpt-table, .wcpt-table-scroll-wrapper-outer.wcpt-device-laptop:visible .frzTbl-table",
      $table;
    return (
      "phone" != device ||
        $wcpt.find(table_selector.replace("laptop", "phone")).length ||
        (device = "tablet"),
      "tablet" != device ||
        $wcpt.find(table_selector.replace("laptop", "tablet")).length ||
        (device = "laptop"),
      $wcpt.find(table_selector.replace("laptop", device))
    );
  }
  ...
  (window.wcpt_cache = {
    ...
    exist: function (url) {
      return (
        ...
        null !== window.wcpt_cache.data[url]
      );
    },
    get: function (url) {
    ...
    set: function (url, cachedData, callback) {
      window.wcpt_cache.remove(url),
        ...
        $.isFunction(callback) && callback(cachedData);
    },
  }),
    ...
    $("body").on("click", ".frzTbl .wcpt-heading.wcpt-sortable", function () {
      var $this = $(this),
        ...
        scrollLeft = $this
          ...
          .siblings(".frzTbl-scroll-overlay")[0].scrollLeft;
      $("body").one(
    ...
    $("body").on("wcpt_layout", ".wcpt", function layout(e, data) {
      ...
      if ($(">.wcpt-device-view-loading-icon", $wrap).length) {
        var url = window.location.href,
          ...
          query_exists = -1 !== url.indexOf("?"),
          query = "",
          device = get_device(),
          replace;
        if ((hash && (url = url.replace(hash, "")), query_exists))
          (re = new RegExp("&*" + id + "_device=(laptop|phone|tablet)", "gm")),
            (url = (
              url.replace(re, "") +
              "&" +
              id +
        ...
        else url = url + "?" + id + "_device=" + device;
        return (
          hash && (url += hash),
          -1 !== $wcpt.attr("data-wcpt-sc-attrs").indexOf("_only_loop")
            ? void (window.location = url)
            : ((query = url.substr(url.indexOf("?") + 1)),
              hash && (query = query.replace(hash, "")),
              void attempt_ajax($wcpt, query, !1, "device_view"))
        );
      }
      $wrap.find(".wcpt-heading").each(function () {
        var $this = $(this);
        $this.find(".wcpt-sorting-icons").length &&
      ...
      var sc_attrs_string = $wcpt.attr("data-wcpt-sc-attrs"),
        sc_attrs =
          sc_attrs_string && "{}" !== sc_attrs_string
            ...
            : {},
        options = {
          left: sc_attrs.laptop_freeze_left
            ? parseInt(sc_attrs.laptop_freeze_left)
            : 0,
          right: sc_attrs.laptop_freeze_right
            ? parseInt(sc_attrs.laptop_freeze_right)
            : 0,
          heading:
            !!sc_attrs.laptop_freeze_heading &&
            "false" !== sc_attrs.laptop_freeze_heading,
          grab_and_scroll: !!sc_attrs.grab_and_scroll,
          wrapperWidth: sc_attrs.laptop_freeze_wrapper_width
            ? parseInt(sc_attrs.laptop_freeze_wrapper_width)
            : 0,
          wrapperHeight: sc_attrs.laptop_freeze_wrapper_height
            ? parseInt(sc_attrs.laptop_freeze_wrapper_height)
            : 0,
          tableWidth: sc_attrs.laptop_freeze_table_width
            ? parseInt(sc_attrs.laptop_freeze_table_width)
            : 0,
          offset: sc_attrs.laptop_scroll_offset
      ...
      (options.breakpoint[wcpt_params.breakpoints.tablet] = {
        left: sc_attrs.tablet_freeze_left
          ? parseInt(sc_attrs.tablet_freeze_left)
          : 0,
        right: sc_attrs.tablet_freeze_right
          ? parseInt(sc_attrs.tablet_freeze_right)
          : 0,
        heading:
          !!sc_attrs.tablet_freeze_heading &&
          "false" !== sc_attrs.tablet_freeze_heading,
        wrapperWidth: sc_attrs.tablet_freeze_wrapper_width
          ? parseInt(sc_attrs.tablet_freeze_wrapper_width)
          : 0,
        wrapperHeight: sc_attrs.tablet_freeze_wrapper_height
          ? parseInt(sc_attrs.tablet_freeze_wrapper_height)
          : 0,
        tableWidth: sc_attrs.tablet_freeze_table_width
          ? parseInt(sc_attrs.tablet_freeze_table_width)
          : 0,
        offset: sc_attrs.tablet_scroll_offset
          ? parseInt(sc_attrs.tablet_scroll_offset)
          : 0,
      }),
        (options.breakpoint[wcpt_params.breakpoints.phone] = {
          left: sc_attrs.phone_freeze_left
            ? parseInt(sc_attrs.phone_freeze_left)
            : 0,
          right: sc_attrs.phone_freeze_right
            ? parseInt(sc_attrs.phone_freeze_right)
            : 0,
          heading:
            !!sc_attrs.phone_freeze_heading &&
            "false" !== sc_attrs.phone_freeze_heading,
          wrapperWidth: sc_attrs.phone_freeze_wrapper_width
            ? parseInt(sc_attrs.phone_freeze_wrapper_width)
            : 0,
          wrapperHeight: sc_attrs.phone_freeze_wrapper_height
            ? parseInt(sc_attrs.phone_freeze_wrapper_height)
            : 0,
          tableWidth: sc_attrs.phone_freeze_table_width
            ? parseInt(sc_attrs.phone_freeze_table_width)
            : 0,
          offset: sc_attrs.phone_scroll_offset
            ? parseInt(sc_attrs.phone_scroll_offset)
            : 0,
        }),
        $table.length &&
          "function" == typeof jQuery.fn.freezeTable &&
          $table.freezeTable(options);
      var device = get_device(),
        $sidebar = $(".wcpt-left-sidebar, .wcpt-was-left-sidebar", $wcpt).not(
          ".wcpt-nav-modal .wcpt-navigation"
        ),
        ft_reload_required = !1,
        $table,
        $rows;
      ("laptop" == device && $sidebar.hasClass("wcpt-was-left-sidebar")
        ? ($sidebar
            .removeClass("wcpt-header wcpt-was-left-sidebar")
            .addClass("wcpt-left-sidebar"),
          (ft_reload_required = !0))
        : "laptop" == device ||
      ...
      ($table = wcpt_get_container_original_table($wcpt)).data(
        "wcpt_checked_rows"
      ) && $table.data("wcpt_checked_rows").length) &&
        $(".wcpt-row", $table).each(function () {
          var $this = $(this),
  ...
  function after_every_load($container) {
    ...
    sc_attrs_attr &&
      ...
      wcpt_get_container_tables($container).each(function () {
        ...
        $cb.each(function () {
          ...
          -1 == col_index.indexOf(_col_index) && col_index.push(_col_index);
        }),
          $.each(col_index, function (key, index) {
            var $heading = $("th", $heading_row).eq(index);
            $heading_row.removeClass("wcpt-hide"),
              $(".wcpt-cart-checkbox-heading", $heading).length ||
                $heading.prepend(
    ...
    if (
      ...
      if (compare_items__string)
        compare_items__string.split(",").forEach(function (item) {
        ...
        $table.each(function () {
          for (
            ...
            var column_cells = $table.find(
              ...
              empty_column_cells = $table
                .find(".wcpt-cell:nth-child(" + column_count + ")")
                .filter(":empty");
            column_cells.length == empty_column_cells.length &&
              column_cells
                .add(
                  $table.find(".wcpt-heading:nth-child(" + column_count + ")")
                )
                .addClass("wcpt-hide"),
              --column_count;
          }
  ...
  function lazy_load_start() {
    window.wcpt_lazy_loaded ||
      ($(".wcpt-lazy-load").each(function () {
        var $this = $(this);
        attempt_ajax($(this), !1, !1, "lazy_load");
      }),
  ...
  function button_click(e) {
    ...
    if ($("body").hasClass("wcpt-photoswipe-visible")) e.preventDefault();
    else if (
      -1 ===
      $.inArray(link_code, [
        "product_link",
      ...
      if ((e.preventDefault(), is_variable)) {
        ...
        if (
          ...
          if (!variation_selected)
            return void alert(wcpt_i18n.i18n_make_a_selection_text);
          if (!variation_found)
            return void alert(wcpt_i18n.i18n_no_matching_variations_text);
          if (!variation_available)
            return void alert(wcpt_i18n.i18n_unavailable_text);
        }
      }
      if (is_variable || !$button.hasClass("wcpt-disabled")) {
        if (has_nyp) {
          ...
          if ($nyp.length) {
            ...
            if (
              ($nyp.val()
                ? min && $nyp.val() < parseFloat(min)
                  ? (error =
                      wcpt_nyp_error_message_templates.minimum_js.replace(
                        "%%MINIMUM%%",
                  ...
                  : max &&
                    ...
                    (error =
                      wcpt_nyp_error_message_templates.maximum_js.replace(
                        "%%MAXIMUM%%",
                        woocommerce_nyp_format_price(
                          max,
                          woocommerce_nyp_params.currency_format_symbol,
        ...
        var $wcpt_qty = $(
          ...
          nyp;
        if (($wc_qty.length && (qty = $wc_qty.val()), $wcpt_qty.length)) {
          var val = parseFloat($wcpt_qty.val());
          (!isNaN(val) && parseFloat($wcpt_qty.val())) ||
            $wcpt_qty
              .filter("input")
              .first()
              .each(function () {
        ...
        if (((params.payload.products[product_id] = qty), has_addons)) {
        ...
        if (has_measurement) {
          ...
          $.isEmptyObject(measurement) ||
            (params.payload.measurement[product_id] = measurement);
        }
        if (has_nyp)
          (nyp = get_nyp($product_rows)) &&
        ...
        if (is_variation) {
          ...
          $missing_attribute_select.length &&
            $missing_attribute_select.each(function () {
              var $this = $(this),
                attribute = $this.attr("data-wcpt-attribute"),
                term = $this.val();
              term && (variation_attributes[attribute] = term);
            }),
            void 0 === params.payload.variations[product_id] &&
        ...
        if (
          (has_addons &&
            ($.isEmptyObject(addons) || $.extend(ajax_data, addons)),
          has_measurement)
        ) {
          var measurement = get_measurement($product_rows);
          $.isEmptyObject(measurement) || $.extend(ajax_data, measurement);
        }
        if (has_nyp) (nyp = get_nyp($product_rows)) && (ajax_data.nyp = nyp);
        if (
          ((is_variable || is_variation) &&
            (variation_id && (ajax_data.variation_id = variation_id),
            variation_attributes && $.extend(ajax_data, variation_attributes)),
          (ajax_data.return_notice = "cart_ajax" == link_code),
          is_composite ||
            is_bundle ||
          ...
          return (
            void 0 !== window.wcpt_product_form[product_id]
  ...
  function is_incomplete_variation(variation_attributes) {
    var is_incomplete_variation = !1;
    return (
      $.each(variation_attributes, function (key, value) {
        if (!value) return (is_incomplete_variation = !0), !1;
      }),
  ...
  function submit_via_post(href, data) {
    ...
    $.each(data, function (key, val) {
      if ("action" != key) {
        ...
        $input.val(val), $form.append($input);
      }
    }),
  ...
  function prep_product_form($modal, $button, pre_select) {
    var link_code = $button.attr("data-wcpt-link-code"),
      href = "cart_ajax" == link_code ? "" : $button.attr("href");
    $modal.on("wcpt_close", function () {
      $modal.remove(), $("body").removeClass("wcpt-modal-on");
    }),
      $(".cart", $modal).each(function () {
        ...
        $form.hasClass("variations_form")
          ...
          $.fn.init_addon_totals && $form.init_addon_totals(),
          "object" == typeof wcPaoInitAddonTotals &&
            wcPaoInitAddonTotals.init($form),
          "undefined" != typeof wcpt_wc_mc_init_cart &&
            $form.each(wcpt_wc_mc_init_cart),
          void 0 !== jQuery.fn.wc_nyp_form && $form.wc_nyp_form(),
            ...
            $.each(pre_select, function (key, val) {
              ...
              if ($control.is("input.qty")) {
                ...
                var min = $control.attr("min")
                    ...
                    : 0,
                  max =
                    !!$control.attr("max") && parseFloat($control.attr("max"));
                (val < min || isNaN(val)) && (val = min),
                  max && val > max && (val = max);
              }
              $control.val(val);
            }),
          ...
          "cart_ajax" == link_code &&
            $form.on("submit", function (e) {
              ...
              $.each($form.serializeArray(), function (i, field) {
                void 0 === external_payload[field.name]
                  ? (external_payload[field.name] = field.value)
                  : ("object" != typeof external_payload[field.name] &&
                ...
                wcpt_cart({
                  ...
                  payload: { variation_form: !0 },
                }),
                $modal.trigger("wcpt_close");
            }),
          wcpt_get_sibling_rows($button.closest(".wcpt-row"))
  ...
  function disable_button($button, add_condition) {
    add_condition && $button.addClass(add_condition),
      $button.addClass("wcpt-disabled");
  }
  function enable_button($button, clear_condition) {
    clear_condition && $button.removeClass(clear_condition),
  ...
  function loading_badge_on_button($button) {
    if (
      ...
      !$button.find(".wcpt-cart-badge-refresh").length)
    ) {
      var svg =
  ...
  function add_count_badge_to_button(in_cart, $button) {
    parseFloat(in_cart)
      ? ($button.find(".wcpt-cart-badge-number").length
  ...
  function search_submit(e) {
    ...
    if (
      ...
      if ($nav_modal.length) return void $(".wcpt-nm-apply").click();
      append
        ? $nav.trigger("change")
        : attempt_ajax($container, query, append, "filter"),
        $nav_modal.length && $nav_modal.trigger("wcpt_close");
    }
  }
  $(window).on("orientationchange", function (e) {
    recent_orientationchange = !0;
  }),
    lazy_load_start(),
    ...
    $("body").on("click", ".wcpt-search-clear", function (e) {
      ...
      $input.val(""),
        $nav_modal.length
          ? $(".wcpt-nm-apply").click()
          : (attempt_ajax($container, query, !0, "filter"),
            $nav_modal.length && $nav_modal.trigger("wcpt_close"));
    }),
    "laptop" !== wcpt_params.initial_device &&
      $("body").on("click", ".wcpt-button[download]", function (e) {
        e.preventDefault();
        var $this,
          url = $(this).attr("href");
        url && window.open(url, "_blank", !1);
      }),
  ...
  function dropdown_mouse_open(e) {
    ...
    if (
      ...
      "click" != wcpt_global_tooltip_trigger_mode
    )
      ...
      } else $this.addClass("wcpt-open"), fix_tooltip_position($this);
  }
  function dropdown_mouse_close(e) {
    ...
    if (
      ...
      if ($this.hasClass("wcpt-tooltip--hover-intent-enabled")) {
        ...
        clear_timeout && clearTimeout(clear_timeout);
      }
  ...
  function dropdown_touch_toggle(e) {
    ...
    if (
      !$dropdown.length ||
      "hover" != wcpt_global_tooltip_trigger_mode ||
  ...
  function fix_tooltip_position($tooltip) {
    ...
    if ($tooltip.hasClass("wcpt-tooltip")) {
      ...
      var container_rect = $.extend({}, $container[0].getBoundingClientRect()),
        ...
        $freezeTable = $(".frzTbl-table", $container);
      ...
      container_rect.width < parseInt(content_rect.width) + 2 * margin &&
        $content.css("max-width", container_rect.width - 2 * margin);
    ...
    if ($tooltip.hasClass("wcpt-tooltip")) {
      var $label = $tooltip.find("> .wcpt-tooltip-label"),
        ...
        $arrow;
      $("> .wcpt-tooltip-arrow", $content).css(
        "left",
  ...
  function apply_nav(e) {
    var $target = $(e.target),
      $container,
      $nav = ($container = $target.closest(".wcpt")).find(".wcpt-navigation");
    if (!$target.closest(".wcpt-search-filter-options").length) {
      ...
      if ($target.closest(".wcpt-range-filter")) {
        ...
        $min.val(min),
          $max.val(max),
          min || (min = $range_slider.attr("min")),
          max || (max = $range_slider.attr("max")),
          $range_slider.val(min + "," + max);
      }
      if (
        !$target.closest(".wcpt-search").length &&
        !$target.closest(".wcpt-nav-modal").length
      ) {
        var $this = $(this),
          $nav,
          $container,
          table_id = ($container = ($nav = $this.add(
            $this.siblings(".wcpt-navigation")
          )).closest(".wcpt"))
            .attr("id")
            .substring(5),
          $nav_clone = $nav.clone();
        ...
        if (!$(e.target).closest('[data-wcpt-filter="sort_by"]').length) {
          ...
          if ($current_sort_col.length)
            query +=
              ...
              table_id +
              "_orderby=column_" +
              $current_sort_col.attr("data-wcpt-column-index") +
              "&" +
              table_id +
              "_order=" +
              ($current_sort_col.find(".wcpt-sorting-icons.wcpt-sorting-asc")
                .length
                ? "ASC"
                : "DESC");
        }
  ...
  function nav_filter_feedback($nav) {
    $(".wcpt-filter", $nav.filter(".wcpt-header")).each(function () {
      var $this = $(this),
        ...
        $checked,
        checked_count = $this.find("input[type=checkbox]:checked").length,
      ...
      if (
        !$this.hasClass("wcpt-options-row") &&
        -1 !=
          $.inArray(filter, [
            "custom_field",
            "attribute",
        ...
        if (
          ...
          if (
            ...
            !$selected.length && "price_range" == filter)
          )
            (label_append =
              "<span>" +
              (min =
              ...
              ($(".wcpt-range-input-min", $this).val() ==
                ...
                ($this.addClass("wcpt-radio-op-selected"), (radio_permit = !0));
          if (!$selected.length && "custom_field" == filter)
            (label_append =
              "<span>" +
              (min = $(".wcpt-range-input-min", $this).val() || 0) +
              " - " +
  ...
  function attempt_ajax($container, new_query, append, purpose) {
    if (void 0 === purpose) throw "WCPT: Define AJAX purpose";
    var query = "",
      earlier_query = $container.attr("data-wcpt-query-string");
    append && earlier_query
    ...
    var parsed_params = parse_query_string(window.location.search.substring(1)),
      query_obj = parse_query_string(query.substring(1));
    void 0 !== window.wcpt_persist_params &&
      $.each(wcpt_persist_params, function (index, i) {
        "undefined" !== parsed_params[i] &&
          void 0 !== parsed_params[i] &&
          void 0 === query_obj[i] &&
    ...
    var device = "laptop",
      ...
      query_obj;
    $scroll_outer.length
      ? $scroll_outer.hasClass("wcpt-device-phone")
        ? (device = "phone")
        : $scroll_outer.hasClass("wcpt-device-tablet") && (device = "tablet")
      : $("body").hasClass("wcpt-nav-modal-on")
      ...
      (query_obj = parse_query_string(query))[table_id + "_device"] !==
        device && (query += "&" + table_id + "_device=" + device);
    var _sc_attrs = $container.attr("data-wcpt-sc-attrs"),
      sc_attrs = _sc_attrs && "{}" !== _sc_attrs ? JSON.parse(_sc_attrs) : {},
      disable_ajax = sc_attrs.disable_ajax && "false" !== sc_attrs.disable_ajax,
      new_query_p = new_query ? parse_query_string(new_query) : {},
      earlier_query_p = earlier_query
        ? parse_query_string(earlier_query.substring(1))
        : {},
    ...
    if (
      (-1 !==
        $.inArray(purpose, ["device_view", "lazy_load", "refresh_table"]) &&
        (scroll = !1),
      "filter" == purpose && (query += "&" + table_id + "_filtered=true"),
      parsed_params[table_id + "_from_shop"] &&
        (query += "&" + table_id + "_from_shop=true"),
      "filter" == purpose && sc_attrs.form_mode)
    ) {
      query = query.split(table_id + "_").join(wcpt_params.shop_table_id + "_");
      var i = wcpt_params.shop_url.indexOf("?lang=");
      return (
        -1 !== i &&
          -1 !== query.indexOf("lang=") &&
          (wcpt_params.shop_url = wcpt_params.shop_url.substring(0, i)),
        (url =
          -1 == wcpt_params.shop_url.indexOf("?")
            ? wcpt_params.shop_url + query
            : wcpt_params.shop_url + "&" + query.slice(1)),
        (url += "&" + wcpt_params.shop_table_id + "_device="),
        (window.location = url),
        void console.log("wcpt notice: redirect to shop")
      );
    }
    ...
    var permit_cache =
      !("refresh_table" == purpose) && "undefined" == typeof WavePlayer;
    if (disable_ajax)
      return (
        (window.location = query), void console.log("wcpt notice: disable ajax")
      );
    $.isEmptyObject(sc_attrs) ||
    ...
    $.ajax({
      ...
      response && -1 !== response.indexOf("wcpt-table")
        ...
        : (console.log("wcpt notice: query fail"), (window.location = query));
    });
  }
  function parse_query_string(query) {
    for (
      var vars = query.split("&"), query_string = {}, i = 0;
      i < vars.length;
      i++
    ) {
      ...
      if (void 0 === query_string[key]) query_string[key] = value;
      else if ("string" == typeof query_string[key]) {
        var arr = [query_string[key], value];
        query_string[key] = arr;
      } else query_string[key].push(value);
    }
    return query_string;
  }
  function ajax_success(response, $container, scroll, device, purpose) {
    ...
    $container.replaceWith($new_container),
      void 0 !== window.wp.mediaelement && window.wp.mediaelement.initialize(),
      $(".cart", $new_container).each(function () {
        var $form = $(this);
        $form.hasClass("variations_form") && $form.wc_variation_form(),
          $.fn.init_addon_totals && $form.init_addon_totals(),
          "object" == typeof wcPaoInitAddonTotals &&
            wcPaoInitAddonTotals.init($form);
      }),
      ...
      offset = {
        laptop:
          ...
          "" == sc_attrs.laptop_scroll_offset
            ...
            : sc_attrs.laptop_scroll_offset,
        tablet:
          void 0 === sc_attrs.tablet_scroll_offset ||
          "" == sc_attrs.tablet_scroll_offset
            ? 20
            : sc_attrs.tablet_scroll_offset,
        phone:
          void 0 === sc_attrs.phone_scroll_offset ||
          "" == sc_attrs.phone_scroll_offset
            ? 20
            : sc_attrs.phone_scroll_offset,
      },
      $scroll_target = $new_container;
    if (scroll) {
      if (sc_attrs[device + "_auto_scroll"]) {
        var offset = offset[device];
        isNaN(offset) &&
          ("string" == typeof offset
            ? (offset = $(offset).height())
            : "object" == typeof offset && (offset = offset.height())),
          $("html, body").animate(
  ...
  function nav_modal(e) {
    ...
    $(".wcpt-nm-sort-placeholder", $nav_modal).replaceWith($sort.clone()),
      ...
      $("body").addClass("wcpt-nav-modal-on").append($nav_modal),
      ...
      $nav_modal.find(".wcpt-nm-reset").on("click", function () {
        var query = $("<form>").append($nav_modal.clone()).serialize(),
          ...
          query = "";
        $nav_modal.trigger("wcpt_close"),
          attempt_ajax($container, "", !1, "filter");
      }),
  ...
  function init_photoswipe($this, index, append_item) {
    if (
      ...
      "undefined" != typeof PhotoSwipeUI_Default
    ) {
      var items = JSON.parse($this.attr("data-wcpt-photoswipe-items")),
        index = void 0 === index ? 0 : parseInt(index);
      append_item && items.push(append_item);
      var index_src = items[index].src,
        unique_src = [],
        _items = [];
      $.each(items, function (index2, item) {
        -1 === $.inArray(item.src, unique_src) &&
          (_items.push(item), unique_src.push(item.src));
      }),
        (items = _items);
      ...
      photoswipe.init(),
        $.each(items, function (index3, item) {
          item.src === index_src && (index = index3);
        }),
  ...
  function position_offset_zoom_container(e, $offset_zoom, $trigger) {
    ...
    rect.right > viewport_width && (position = "left"),
      "left" == position &&
    ...
    if (!window.wcpt_form_reset_flag) {
      ...
      $.each(variations, function (index, value) {
        if (parseInt(value.variation_id) == selected_variation)
          return (variation = value), !1;
      });
      var variation_selected = !0;
      $(".variations select", $form).each(function () {
        var $this = $(this);
        (attributes[$this.attr("name")] = $this.val()),
          $this.val() || (variation_selected = !1);
      });
      var variation_available = !1;
      variation &&
  ...
  function prep_variation_options($container) {
    $(".wcpt-product-type-variable", $container).each(function () {
      var $row = $(this),
        ...
        $options;
      $dropdown.add($radio).add($form).length &&
        $row.data("wcpt_variation_ops", !0),
        $form.length
          ? $form.each(function () {
              var $form = $(this);
              setTimeout(function () {
          ...
          : $radio.length && $radio.filter(":checked").trigger("change");
    });
  }
  ...
  if (
    ...
    $("body").on("click mouseover", ".wcpt-tooltip-content", function () {
      ...
      $tooltip.hasClass("wcpt-tooltip--hover-disabled") &&
    ...
    $("body").on(
      ...
      function (e) {
        var $this,
          ...
          code;
        13 == (e.keyCode ? e.keyCode : e.which) && $filters.trigger("change");
      }
    ),
    ...
    $("body").on("click", ".wcpt-clear-filter", function (e) {
      ...
      if ("search" == filter)
        ...
        $inputs = $filter.hasClass("wcpt-range-filter")
          ? $filter.find("input")
          : $navs
              .find(
                '.wcpt-filter[data-wcpt-filter="' +
      ...
      $inputs
        ...
        $inputs
          .filter(":input[type=text], :input[type=number], :input[type=search]")
          .val(""),
        $navs.first().trigger("change"),
        $clear_filter.siblings(".wcpt-clear-filter").length
          ? $clear_filter.remove()
    ...
    (window.wcpt_column_heading_sort_handler = function () {
      ...
      if ($sorting.length) {
        var order = $sorting.hasClass("wcpt-sorting-asc") ? "desc" : "asc",
          ...
          query;
        if (
          $(".wcpt-sorting-" + order + "-icon", $sorting).hasClass("wcpt-hide")
        ) {
          if (
    ...
    $("body").on(
      ...
      function (e) {
        ...
        var $this = $(this),
          ...
          url = $this.attr("href"),
          index = url.indexOf("?"),
          params = -1 != index && parse_query_string(url.slice(index + 1)),
          page = params ? params[table_id + "_paged"] : 1,
          query;
        (append = !0),
          attempt_ajax(
            $container,
            table_id + "_paged=" + page,
            append,
            "paginate"
          );
      }
    ...
    (window.wcpt_update_cart_items = function (cart) {
      ...
      $.each(cart, function (key, item) {
        cart_products[item.product_id] || (cart_products[item.product_id] = 0),
          item.variation_id &&
            !cart_products[item.variation_id] &&
    ...
    $("body").on(
      "click",
      ".wcpt-left-sidebar .wcpt-filter > .wcpt-filter-heading",
      function (e) {
        var $this, $filter;
        $(e.target).closest(".wcpt-tooltip").length ||
          $(this).closest(".wcpt-filter").toggleClass("wcpt-filter-open");
      }
    ),
    $("body").on(
      "click",
      ".wcpt-left-sidebar .wcpt-filter:not(.wcpt-filter-open)",
      function (e) {
        var $this = $(this);
        e.target === this && $this.addClass("wcpt-filter-open");
      }
    ),
    ...
    $("body").on("click", ".wcpt-tg-trigger", function () {
      ...
      $toggle.toggleClass(" wcpt-tg-on wcpt-tg-off "),
        ft && $table.freezeTable("cell_resize");
    }),
    ...
    $("body").on("click", ".pswp__container", function (e) {
      ...
      window.innerWidth < 720 &&
        photoswipe &&
        ($(e.target).closest(".pswp__button").length || photoswipe.close());
    }),
    $("body").on("click", ".wcpt-gallery__item", function () {
      ...
      $gallery.hasClass("wcpt-gallery--include-featured") || (index += 1),
        init_photoswipe($gallery, index);
    }),
    $("body").on("click", ".wcpt-gallery a", function (e) {
      var $this, $gallery;
      e.preventDefault(), init_photoswipe($(this).closest(".wcpt-gallery"));
    }),
    $("body").on("click", ".wcpt-lightbox-enabled", function () {
      ...
      if (
        ...
        var index = 0,
          ...
          append_item = !1;
        if (
          "variable" === $row.attr("data-wcpt-type") &&
          ...
          $.each(pswp_items, function (_index, item) {
            if (item.src == src) return (index = _index), (found = !0), !1;
          }),
            found ||
              ((append_item = {
                src: variation.image.full_src,
          ...
          $this.attr("data-wcpt-photoswipe-items") &&
            $.each(pswp_items, function (_index, item) {
              if (item.src == src) return (index = _index), !1;
            });
    ...
    $("body").on(
      ...
      function (e) {
        ...
        $this.closest(".frzTbl--grab-and-scroll--grabbing").length ||
          (destroy_offset_zoom_containers(),
          $wcpt
            .append($offset_zoom)
    ...
    $("body").on(
      ...
      function (e, data) {
        var $row = get_product_rows($(this)),
          $variation_dropdown = $row.find(".wcpt-select-variation-dropdown");
        if (
          ...
          $row.find(".variations_form").each(function () {
            ...
            (current_variation_id = $(".variation_id", $this).val()),
              data.variation_id != current_variation_id &&
                ((window.wcpt_form_reset_flag = !0),
                $(".reset_variations", $this).trigger(
                  "click.wc-variation-form"
                ),
                (window.wcpt_form_reset_flag = !1),
                data.variation_id &&
                  ($(".variations select", $this).each(function () {
                    var $this = $(this),
                      name = $this.attr("name");
                    void 0 !== data.attributes[name]
            ...
            ($qty = $row.find(".wcpt-quantity input[type=number].qty"))
              ...
              $qty.each(function () {
                var $this = $(this),
                  ...
                  value = "";
                (value =
                  "min" == inital_value ? min : "0" === inital_value ? 0 : ""),
                  "min" === inital_value &&
                    $this.attr("data-wcpt-reset-on-variation-change") &&
                    (value = min),
                  $this.attr({ min: "", max: "", step: "", value: value }),
                  $this.val(value),
          ...
          $product_image_wrapper.length &&
              ...
              .each(function () {
                ...
                stock &&
                  ...
                  $.each(parsed_rules, function (index, rule) {
                    rule[0] <= stock &&
                      rule[1] >= stock &&
                      ((label = rule[2]), (found_rule = !0));
                  }),
                  !found_rule && stock < 0 && ((stock = ""), (label = "")),
            ...
            $row
              ...
              .each(function () {
                ...
                if (
                  void 0 !== window["wcpt_" + table_id + "_variable_switch_cf"]
                ) {
                  var cf_vals =
                    window["wcpt_" + table_id + "_variable_switch_cf"];
                  void 0 !== cf_vals[element_id] &&
        ...
        if (!$.isEmptyObject(data.variation)) {
          var $button, $qty;
          enable_button(
            ($button = $row.find('[data-wcpt-link-code^="cart"]')),
            "wcpt-no-variation-selected"
          ),
          ...
          if ($select.length) {
            var qty_label = $select.attr("data-wcpt-qty-label"),
              ...
              options =
                ...
                "</option>";
            for (
              data.variation.max_qty && (max_qty = data.variation.max_qty);
              val < max_qty;

            )
              options +=
                "<option>" + (val += data.variation.step || 1) + "</option>";
          ...
          $product_image[0]
            ...
            data.variation.sku &&
              $row.find(".wcpt-sku").each(function () {
                var $this = $(this);
                $this.hasClass("wcpt-variable-switch") &&
                  ...
                  data.variation.is_in_stock
                    ? ((data.variation.managing_stock &&
                      ...
                      ? ((message_tpl = "in_stock_managed_message"),
                        ...
                        data.variation.backorders_allowed
                          ? data.variation.backorders_require_notification &&
                            on_backorder_managed_message
                            ? ((message_tpl = "on_backorder_managed_message"),
                              (stock_class = "wcpt-on-backorder"))
                            : data.variation.stock <= 0
                            ? ((message_tpl = "on_backorder_message"),
                              (stock_class = ""),
                              $this.hide())
                            : ((message_tpl = "in_stock_message"),
                              (stock_class = "wcpt-in-stock"))
                          : 1 == data.variation.stock && single_stock_message
            ...
            $row
              ...
              .each(function () {
                ...
                if (
                  ...
                  void 0 !== cf_vals[element_id] &&
                    $this.html(
                      cf_vals[element_id][data.variation.variation_id]
                    );
                }
              });
        }
        var $freeze_table_container,
          $freeze_table = $row.closest(".frzTbl").find(".frzTbl-table");
        $freeze_table.length && $freeze_table.freezeTable("cell_resize");
      }
    ),
    ...
    $(".wcpt").length && $(document).off("change", ".qty"),
  ...
  function limit_qty_controller($qty_wrapper) {
    $qty_wrapper.each(function () {
      var $this = $(this),
        ...
        max = !!$qty.attr("max") && parseFloat($qty.attr("max")),
        step = $qty.attr("step") ? parseFloat($qty.attr("step")) : 1,
        val = parseFloat($qty.val());
      (val && !isNaN(val)) || (val = 0),
        -1 !== $.inArray(initial, ["empty", "0"]) && (min = 0),
        $minus.removeClass("wcpt-disabled"),
        val - step < min && $minus.addClass("wcpt-disabled"),
        $plus.removeClass("wcpt-disabled"),
        !1 !== max && val + step > max && $plus.addClass("wcpt-disabled");
    });
  }
        ...
        (price =
          ...
          $rows.data("wcpt_variation").display_price
            ? parseFloat(
                $rows
                  .data("wcpt_variation")
                  .display_price.split(wcpt_params.price_decimal_separator)
                  .join(".")
                  .split(wcpt_params.price_thousand_separator)
                  .join("")
              )
            : 0),
      $row.hasClass("wcpt-product-has-name-your-price"))
    ) {
      var $nyp = $(".wcpt-name-your-price--input", $rows);
      $nyp.filter(":visible").length &&
        (price = $nyp.val() ? parseFloat($nyp.val()) : 0);
    }
    (total = qty * price),
      $cb.length &&
        !$rows.data("wcpt_checked") &&
        "min" !== $qty.attr("data-wcpt-initial-value") &&
        (total = 0),
      $total.each(function () {
        var $this = $(this),
          _total = total;
        $this.hasClass("wcpt-total--include-total-in-cart") &&
          (_total += prev_total),
          _total
            ? $this
  ...
  function wcpt_get_sibling_tables($table) {
    ...
    return $freeze_table.length ? $(".wcpt-table", $freeze_table) : $table;
  }
  function wcpt_get_original_row($row) {
    var $sibling_rows;
    return (
      wcpt_get_sibling_rows($row).each(function () {
        var $row = $(this);
        if (!$row.closest("table").hasClass("frzTbl-clone-table"))
          return ($original = $row), !1;
      }),
      $original
    );
  }
  function wcpt_checkbox_trigger_init() {
    ...
    if ($checkbox_trigger.length) $checkbox_trigger.removeClass("wcpt-hide");
    else {
    ...
    if (
      ...
      $checked_rows.each(function () {
        ...
        isNaN(val) || (qty = (1e12 * val + 1e12 * qty) / 1e12);
      }),
        $checkbox_trigger
          .data({ wcpt_checked_rows: $checked_rows, wcpt_qty: qty })
    ...
    } else $checkbox_trigger.hide();
  }
  function update_table_add_selected_to_cart() {
    var _ = this;
    setTimeout(function () {
      var $this,
        $container = $(_).closest(".wcpt"),
  ...
  function format_price_figure(price) {
    return (price = (
      (price =
        (price = parseFloat(price)) !== parseInt(price)
          ? parseFloat(price).toFixed(wcpt_params.price_decimals)
          : parseInt(price)) + ""
    ).replace(".", wcpt_params.price_decimal_separator));
  }
  function format_price(num) {
    return num || "0" === num || 0 === num
      ? wcpt_params.price_format
    ...
    $add_checked.length &&
      ...
      $pagination.before(function () {
        ...
        return (
          ...
          $add_checked.closest(".wcpt-right").length &&
  ...
  function wcpt_cart_checkbox() {
    ...
    $table.each(function () {
      ...
      $checked_rows.each(function () {
        ...
        } else $row.hasClass("wcpt-product-type-variation") && ((variation_id = $row.attr("data-wcpt-variation-id")), (variation_attributes = JSON.parse($row.attr("data-wcpt-variation-attributes"))));
        variation_id &&
          (variations[product_id] || (variations[product_id] = {}),
          ...
          $row.hasClass("wcpt-product-has-measurement") &&
            (measurement[product_id] = get_measurement($row)),
          $row.hasClass("wcpt-product-has-name-your-price") &&
            (nyp[product_id] = get_nyp($row));
      }),
      $checked_rows.trigger("_wcpt_checkbox_change", !1),
      wcpt_cart({
        payload: {
          products: products,
          addons: addons,
  ...
  function get_nyp($row) {
    var $nyp = get_nyp_input_element($row),
      val = 0;
    return $nyp.length && (val = $nyp.val()), val;
  }
  ...
  function dynamic_filters_lazy_load__fetch($filter, key, options) {
    ...
    $.each(options, function (i, option) {
      filter != option.filter ||
        ("attribute" === filter && taxonomy != option.taxonomy) ||
        ...
        success: function (result) {
          ...
          (sc_attrs = $container.attr("data-wcpt-sc-attrs")),
            sc_attrs &&
              ...
              sc_attrs.dynamic_recount && (add_count = !0)),
            add_count &&
              $.each(result.options, function (i, option) {
                var count =
                  ...
                  $prev_count;
                $(".wcpt-count", $label).remove(),
                  $icon.length ? $icon.before(count) : $label.append(count);
              }),
  ...
  function cart_widget_permitted() {
    ...
    return (
      !(!wcpt_params.cart_widget_enabled_site_wide && !$(".wcpt").length) &&
      (wcpt_params.cart_widget_include_urls.trim() &&
        (include_urls = wcpt_params.cart_widget_include_urls
          .trim()
          .replace(/\r\n/g, "\n")
          .split("\n")),
      wcpt_params.cart_widget_exclude_urls.trim() &&
      ...
      !(
        ...
        $.each(include_urls, function (i, path) {
          if (
            (0 === (path = path.trim()).indexOf(site_url) &&
            ...
            if ("/*" != path.trim())
              if (
                "*" ===
                (path = path.replace(/(^\s*\/)|(\/\s*$)/g, "")).substr(-1)
              ) {
                var remove = 1;
                "/*" === path.substr(-2) && (remove = 2),
                  -1 !==
                    url.indexOf(
                      site_url + path.substring(0, path.length - remove) + "/"
                    ) && (include_match = !0);
              } else url == site_url + (path += "/") && (include_match = !0);
            else include_match = !0;
          else url === site_url && (include_match = !0);
        }),
        !include_match)
      ) &&
        (!exclude_urls.length ||
          ...
          $.each(exclude_urls, function (i, path) {
            if (
              ...
              if ("/*" != path.trim())
                ...
                } else url == site_url + (path += "/") && (exclude_match = !0);
              else exclude_match = !0;
            else url === site_url && (exclude_match = !0);
          }),
          !exclude_match)))
    );
  }
  function nyp_validate($nyp) {
    var $nyp_wrapper = $nyp.parent(),
      ...
      $error = "",
      $row,
      checked = $nyp.closest(".wcpt-row").data("wcpt_checked");
    ...
    (checked && !nyp_val && nyp_min)
      ? ($nyp_wrapper.addClass(
        ...
        (message_template = ($error = $(
          ...
          $nyp_wrapper
        )).attr("data-wcpt-error-message-template")),
        $error.text(
          message_template.replace("[min]", format_price_figure(nyp_min))
        ))
      : nyp_val && nyp_max && parseFloat(nyp_val) > parseFloat(nyp_max)
      ? ($nyp_wrapper.addClass(
  ...
  $("body").on(
    mousedown + ("touchstart" == mousedown ? " dblclick" : ""),
      ...
      if (
        ...
        if (
          (!step % 1
            ...
            : ((min = parseFloat(min)),
              ...
              (val = parseFloat(val))),
          isNaN(val) && (val = 0),
          $this.hasClass("wcpt-plus"))
        )
          $qty.val((1e12 * val + 1e12 * step) / 1e12).change();
        else {
          var next = (1e12 * val - 1e12 * step) / 1e12;
          next < min && (next = 0), $qty.val(next).change();
        }
        var count = 0,
          clear = setInterval(function () {
            if (
              !(++count % 5 || count < 50 || $this.hasClass("wcpt-disabled"))
            ) {
              var val = $qty.val() ? $qty.val() : min;
              if (
                ((val = !step % 1 ? parseInt(val) : parseFloat(val)),
                $this.hasClass("wcpt-plus"))
              )
                $qty.val((1e12 * val + 1e12 * step) / 1e12).change();
              else {
        ...
        $this.data("wcpt_clear", clear),
          $this.one(mouseup + " mouseout", function () {
            ...
            clear && (clearInterval(clear), $this.data("wcpt_clear", !1)),
        ...
        if (
          (val ||
            ("min" === initial
              ...
              : "empty" === initial
              ? $this.val("")
              : "0" === initial && $this.val("0")),
          val && val < min && $this.val(min),
          val > max && $this.val(max),
          val && (1e12 * val) % (1e12 * step))
        ) {
          var _val,
            _step,
            new_val = (1e12 * val + 1e12 * step - (val % step) * 1e12) / 1e12;
          $this.val(new_val);
        }
        syncing || $this.trigger("change", !0),
    ...
    $("body").on(
      ...
      function () {
        $(this)
          .find('.wcpt-quantity input[type="number"].qty')
          .each(function () {
            var $this = $(this),
              initial_value = $this.attr("data-wcpt-initial-value");
            if ("0" === initial_value) $this.val(0);
            else if ("empty" === initial_value) $this.val("");
            else if ("min" === initial_value) {
    ...
    $("body").on("keyup change", ".wcpt-quantity .qty", function (e) {
      ...
      if (
        ...
        var $row,
          $button = wcpt_get_sibling_rows($this.closest(".wcpt-row")).find(
            '.wcpt-button[data-wcpt-link-code^="cart_"]'
          );
        $wrapper.hasClass("wcpt-quantity-error")
    ...
    $("body").on(
      ...
      function (e, syncing) {
        ...
        if (
          ($siblings.val(val),
          !$input.closest(".wcpt-add-to-cart-wrapper").length)
        ) {
          ...
          $wc_default_button.data("quantity", val),
            ...
            syncing || ((syncing = !0), $siblings.trigger("change", syncing));
        }
      }
    ),
    ...
    $("body").on(
      ...
      function (e) {
        var $nyp = $(this),
          ...
          qty_val = $qty.val(),
          $cb = $(".wcpt-cart-checkbox ", $rows);
        if ((nyp_validate($nyp), nyp_val && !qty_val))
          $qty.val($qty.attr("min"));
        else if (!nyp_val && qty_val) {
          var inital_val_type = $qty.attr("data-wcpt-nyp-initial-value"),
            val = $qty.attr("min");
          "empty" === inital_val_type
            ? (val = "")
            : "0" === inital_val_type && (val = "0"),
            $qty.val(val);
        }
        update_row_total($(this).closest(".wcpt-row")),
          $qty.length
            ? $qty.change()
            : $cb.length && $rows.trigger("wcpt_checkbox_change", !!$nyp.val());
      }
    ),
    $("body").on("click", ".wcpt-player__button", function () {
      ...
      $button.hasClass("wcpt-player__play-button"))
        ? ($el[0].play(),
          $container.hasClass("wcpt-media-loaded") ||
          ...
          $(".wcpt-player.wcpt-player--playing")
            .not($container)
            .find(".wcpt-player__pause-button")
            .click())
        : $el[0].pause();
      $container.toggleClass("wcpt-player--playing");
    }),
    $("body").on(
      ...
      function () {
        var $this = $(this),
          ...
          $container,
          $nav = $this.closest(".wcpt").find(".wcpt-navigation"),
          $option = $nav.find(
            '[data-wcpt-taxonomy="' +
              taxonomy +
              '"] [data-wcpt-slug="' +
    ...
    $("body").on(
      ...
      function () {
        var $this = $(this),
          ...
          variation_id = $row.attr("data-wcpt-variation-id"),
          params = {
            payload: { products: {}, variations: {}, overwrite_cart_qty: !0 },
          };
        $this.hasClass("wcpt-refresh-enabled") &&
    ...
    (window.wcpt_get_sibling_rows = function ($row) {
      ...
      if (!$freeze_table.length) return $row;
    ...
    $("body").on("change", ".wcpt-cart-checkbox", function (e) {
      ...
      $this
        .closest(".wcpt-row")
        .trigger("_wcpt_checkbox_change", $this.prop("checked"));
    }),
    $("body").on("_wcpt_checkbox_change", ".wcpt-row", function (e, state) {
      ...
      if ($sibling_cbs.length && $sibling_cbs.not(":disabled").length) {
        ...
        var $table_checked_rows = $table.data("wcpt_checked_rows")
          ? $table.data("wcpt_checked_rows")
          : $();
        ($table_checked_rows = state
          ? $table_checked_rows.add($original_row)
          : $table_checked_rows.not($original_row)),
    ...
    $("body").on("select_variation", ".wcpt-row", function (e) {
      ...
      $cb.removeAttr("disabled"),
        ...
        (selected_variation &&
          ...
          ($this.trigger("wcpt_checkbox_change", !1),
          $cb.attr("disabled", !0),
    ...
    setTimeout(function () {
      $("body").on(
        ...
        function (e) {
          var $this = $(this),
            $row,
            $original_row = wcpt_get_original_row($this.closest(".wcpt-row"));
          $this.closest("form.cart").length ||
            ("min" === $this.attr("data-wcpt-initial-value") &&
              !$original_row.data("wcpt_checked")) ||
    ...
    $("body").on("wcpt_checkbox_change", ".wcpt-row", function (e, state) {
      ...
      $sibling_qty.each(function () {
        ...
        state
          ? (val && "0" !== val && !isNaN(val)) || (val = min)
          : (val = "empty" == initial_qty ? "" : "0" == initial_qty ? 0 : min),
          $sibling_qty.val(val),
    ...
    $("body").on("click", ".wcpt-cart-checkbox-heading", function () {
      var $this = $(this),
        ...
        $container,
        $table = wcpt_get_container_original_table($this.closest(".wcpt")),
        $rows;
      $(".wcpt-row", $table).trigger("_wcpt_checkbox_change", state),
    ...
    $("body").on("change", ".wcpt-cart-checkbox", function (e, wcpt_sync) {
      ...
      if (
        ...
        $last_clicked.length &&
          "undefined" != typeof wcpt_shiftKey &&
          wcpt_shiftKey)
      ) {
        ...
        $cb
          .filter(function () {
            var $this = $(this),
              index = $cb.index($this);
            return !$this.prop("disabled") && index >= min && index <= max;
          })
    ...
    (window.wcpt_get_addons = function ($row) {
      var $form = $(
        ...
        addons = {},
        $wcpa_fields = $form.find(".wcpa_form_outer");
            ...
            if (-1 !== suffix_index) {
              if (
                ...
                return (
                  ...
                  void addons[field_name].push(field.value)
                );
              addons[field_name] = field.value;
            } else addons[field_name] = field.value;
          });
      } else
        $.each($form.serializeArray(), function (i, field) {
          var field_name = field.name;
          "[]" == field_name.slice(-2) &&
            ...
            void 0 === addons[field_name]
              ? (addons[field_name] = field.value)
              : ("object" != typeof addons[field_name] &&
                  (addons[field_name] = [addons[field_name]]),
    ...
    $("body").on("wcpt_after_every_load", ".wcpt", function () {
      var $this = $(this),
        $nav_dropdown;
      $(".wcpt-dropdown.wcpt-filter", $this).each(function () {
        var $this = $(this);
        if (
          ...
          $menu.prepend(
            '<input type="text" class="wcpt-search-filter-options" placeholder="' +
              placeholder +
              '" />'
          ),
    ...
    $("body").on("keyup input", ".wcpt-search-filter-options", function (e) {
      ...
      if ((e.preventDefault(), !val))
        return (
          $option.show(),
          $(".wcpt-ac-open", $filter).removeClass("wcpt-ac-open"),
          void $("input[type=radio], input[type=checkbox]", $option).each(
            function () {
              var $this = $(this);
              $this.is(":checked") &&
                $this
      ...
      $option.each(function () {
        var $this = $(this),
          label;
        -1 == $this.text().toLowerCase().trim().indexOf(val)
          ? $this.hide()
          : ($this.show(),
            $this
              .closest(".wcpt-dropdown-option.wcpt-accordion")
    ...
    $("body").on("submit", ".wcpt-global-search__form", function (e) {
      ...
      if (keyword) {
        ...
        else if ("shop" == redirect) {
          e.preventDefault();
          var url = action + "?s=" + keyword;
          category && (url += "&wcpt_search_category=" + category),
            (window.location = url);
      ...
      } else "category" !== clear_redirect && $select.val("").change(), clear_redirect_url && (e.preventDefault(), (window.location = clear_redirect_url));
    }),
    ...
    $("body")
      .on("focus", ".wcpt-global-search__keyword-input", function () {
        ...
        $(this)
          .parent()
          .addClass("wcpt-global-search__keyword-input-wrapper--focus");
      })
      .on("blur", ".wcpt-global-search__keyword-input", function () {
        var $this, $wrapper;
        $(this)
          .parent()
          .removeClass("wcpt-global-search__keyword-input-wrapper--focus");
      }),
    (window.wcpt_cart = function (params) {
      ...
      if (
        ...
        void 0 !== params.external_payload &&
          ...
          $.each(product_request.products, function (id, qty) {
            if (id == product_id) {
              if (
                ...
                product_request.variations[product_id] &&
                void 0 === product_request.variations[product_id][variation_id]
              )
                return;
              if ((loading_badge_on_button($button), 0 === qty && in_cart))
                $row.addClass("wcpt-removing-product");
              else {
                $row.addClass("wcpt-adding-product");
                var $qty,
                  $wcpt_qty = ($qty = $("input.qty", $row)).not(".cart .qty"),
                  initial = $wcpt_qty.attr("data-wcpt-initial-value"),
                  min = parseFloat($wcpt_qty.attr("data-wcpt-min")),
                  return_to_initial;
                $wcpt_qty.attr("data-wcpt-return-to-initial") &&
                  ($sibling_rows.trigger("_wcpt_checkbox_change", !1),
                  "min" == initial
                    ? $qty.val(min)
                    : "0" === initial
                    ? $qty.val(0)
                    : "empty" === initial && $qty.val(""));
                var $wrapper = $qty.closest(".wcpt-quantity");
                $wrapper.length && limit_qty_controller($wrapper);
                var $qty,
                  $first = ($qty = $("select.wcpt-qty-select", $row)).children(
                    "option:first-child"
                  );
              ...
              var $total = $(".wcpt-total", $sibling_rows),
                _qty = variation_id
                  ...
                  : qty;
              $total.length && update_row_total($sibling_rows, _qty);
            }
          });
      });
      var data = $.extend(
        {},
        { wcpt_payload: params.payload, lang: wcpt_i18n.lang },
        params.external_payload ? params.external_payload : {}
      );
      $("body").trigger("wcpt_cart_request", data),
        params.payload.use_cache && window.wcpt_cart_result_cache
          ...
          : $.post(
              ...
              params.always && params.always(result),
                params.redirect &&
    ...
    $("body").on("wcpt_cart", function (e, result) {
      if (
        (result.payload.wcpt_cart_call_id === window.wcpt_cart_call_id &&
      ...
      result.payload.products &&
        ...
        $.each(result.payload.products, function (product_id, product_qty) {
          var variation_id = (variation_qty = !1);
          result.payload.variations &&
            void 0 !== result.payload.variations[product_id] &&
        ...
        result.payload.variation_form && (added = !0);
      ...
      if (
        (result.in_cart &&
          $.each(result.in_cart, function (key, val) {
            ...
            } else in_cart_products.push({ id: key, type: "simple", quantity: val, total: result.in_cart_total[key] });
          }),
        $(".wcpt-row").each(function () {
          var $row = $(this),
        ...
        result.payload &&
          result.payload.products &&
          1 === Object.keys(result.payload.products).length &&
    ...
    $("body").on(
      ...
      function (e, fragment, cart_hash, $button, trigger_lock) {
        trigger_lock || wcpt_cart({ payload: { skip_cart_triggers: !0 } });
      }
    ),
    ...
    $("body").on(
      ...
      function () {
        ...
        if (
          ...
          min_val > max_val &&
            ($this.hasClass("wcpt-range-input-min")
              ? $max.val(min_val)
              : $min.val(max_val));
    ...
    $("body").on("input change", ".wcpt-range-slider", function () {
      var $this = $(this),
        ...
        step;
      max + parseFloat($this.attr("step")) > permitted_max &&
        (max = permitted_max),
        $min.val(min),
        $max.val(max);
    }),
    $("body").on("before_freeze_table_build", ".wcpt-table", function () {
      var $this = $(this),
        $radio_set;
      $(".wcpt-select-varaition-radio-multiple-wrapper", $this).each(
        function () {
          var $this = $(this),
    ...
    $("body").on("after_freeze_table_build", ".wcpt-table", function () {
      ...
      $radio_set.each(function () {
        ...
        $radio.attr("name", name + Math.floor(1e10 * Math.random())),
    ...
    $("body").on(
      ...
      function (e, data) {
        ...
        if (($items.hide(), $row.data("wcpt_variation_selected"))) {
      ...
      function (e, data) {
        ...
        if (input) {
          if (
            ...
            min
              ? $input.attr({ min: min, "data-wcpt-nyp-minimum-price": min })
              : $input.attr({ min: 0, "data-wcpt-nyp-minimum-price": 0 }),
            max
              ? ($input.attr("max", max),
                $input.attr({ max: max, "data-wcpt-nyp-maximum-price": max }))
              : $input.attr({ max: "", "data-wcpt-nyp-maximum-price": "" }),
            initial_value_field && !$input.val())
          ) {
        ...
        min
          ? ($row
              .find(".wcpt-name-your-price--minimum")
          ...
          max
            ? ($row
                .find(".wcpt-name-your-price--maximum")
                .removeClass("wcpt-hide"),
              $row
                .find(".wcpt-name-your-price--maximum .wcpt-amount")
                .text(max))
            : $row.find(".wcpt-name-your-price--maximum").addClass("wcpt-hide"),
          suggested
            ? ($row
                .find(".wcpt-name-your-price--suggested")
                .removeClass("wcpt-hide"),
              $row
    ...
    $("body").on("wcpt_checkbox_change", ".wcpt-row", function (e, checked) {
      var $this = $(this),
        $rows,
        $nyp = get_nyp_input_element(
          wcpt_get_sibling_rows($this.closest(".wcpt-row"))
        );
      checked ? nyp_validate($nyp) : $nyp.val() || nyp_hide_error($nyp);
    });
    ...
    ft_check__maybe_cell_resize(),
      --ft_check__count &&
        (3 == ft_check__count
          ...
          : setTimeout(ft_check__go, 5e3));
  }
  function ft_check__maybe_cell_resize($tables) {
    $tables || ($tables = $(".wcpt-table:not(.frzTbl-clone-table)")),
        ...
        $this.is(":visible") &&
          (freezeTable
            ? freezeTable.env &&
              ...
              $this.height() !== freezeTable.env.tableHeight &&
      ...
      if (
        ($.each(WavePlayer.instances, function (_instance_index, _instance) {
          ...
          $_instances.is(":visible") &&
            ...
            $_instances.closest(".wcpt-row").length &&
            $.each(_instance.tracks, function (_track_index, _track) {
              _track.file === WavePlayer.persistentTrack.file &&
                ...
                ((instance = _instance), (track_index = _track_index));
            });
        }),
        instance)
      )
  ...
  function waveplayer_active_ui_feedback() {
    ...
    $rows
      ...
      $(".wcpt-row")
        ...
        .css({ background: "", outline: "" });
  }
  function sonaar_player_auto_status() {
    if ("undefined" != typeof IRON) {
  ...
  if (
    ...
    $("body").on("wcpt_cart_request", function (e, data) {
      data.wcpt_payload.skip_cart_triggers ||
        ...
        $(".wcpt").each(function () {
          ...
          sc_attrs.refresh_table
            ? $container.addClass("wcpt-loading wcpt-refreshing")
            : sc_attrs.block_table && $container.addClass("wcpt-loading");
        });
    }),
    $("body").on("wcpt_cart", function (e, result) {
      result.payload.skip_cart_triggers ||
        ...
        $(".wcpt").each(function () {
          ...
          sc_attrs.refresh_table
            ? ($.each(window.wcpt_cache.data, function (key, val) {
                (-1 === key.indexOf("?" + table_id + "_") &&
                  -1 === key.indexOf("&" + table_id + "_")) ||
                  delete window.wcpt_cache.data[key];
              }),
              attempt_ajax($container, "", !0, "refresh_table"))
            : sc_attrs.block_table && $container.removeClass("wcpt-loading");
        });
    }),
    ...
    $("body").on("wcpt_layout", ".wcpt", function () {
      ...
      if ("undefined" != typeof WavePlayer) {
        if ((WavePlayer.loadInstances(), !$wcpt_waveplayer_preserve.length))
          return;
    ...
    $("body").on("added_to_cart", function (e, fragment, cart_hash, $button) {
      if ("undefined" != typeof WavePlayer && $button) {
        var product_id = $button.attr("data-product_id");
        void 0 !== WavePlayer.updateTrackCartStatus &&
    ...
    $("body").on("click", ".elementor-tab-title", function () {
      var $this, $container, $wcpt;
      $(this)
        .closest(".elementor-widget-container")
        .find(".wcpt")
        .trigger("wcpt_layout", { source: "elementor__tab" });
    }),
    $("ul.et_pb_tabs_controls > li").on("click", function () {
      var $this = $(this),
        ...
        $container,
        $wcpt = $this
          .closest(".et_pb_tabs_controls")
          .siblings(".et_pb_all_tabs")
          .children()
          .eq(index)
          .find(".wcpt");
      setTimeout(function () {
    ...
    $(".et_pb_toggle_title").on("click", function () {
      var $this,
        $content,
        $wcpt = $(this).next(".et_pb_toggle_content").find(".wcpt");
      setTimeout(function () {
        $wcpt.trigger("wcpt_layout", { source: "divi__tab" });
      }, 700);
    }),
    $("body").on("click", ".fl-accordion-button, .fl-tabs-label", function () {
      var $this, $container, $wcpt;
      $(this)
        .closest(".fl-accordion-item, .fl-tabs-panel")
        .find(".wcpt")
        .trigger("wcpt_layout", { source: "beaver_builder__tab" });
    }),
    $("body").on("click", ".su-tabs-nav span", function () {
      ...
      $this
        .closest(".su-tabs-nav")
        .siblings(".su-tabs-panes")
        .children()
        .eq(index)
        .find(".wcpt")
        .trigger("wcpt_layout", { source: "shortcode_ultimate__tab" });
    }),
    $("body").on("click", ".su-spoiler-title", function () {
      ...
      $(this)
        .closest(".su-spoiler")
        .find(".wcpt")
        .trigger("wcpt_layout", { source: "shortcode_ultimate__accordion" });
    }),
    $("body").on("click", ".vce-classic-tabs-tab", function () {
      var $this = $(this),
        ...
        $container,
        $wcpt = $this
          .closest(".vce-classic-tabs-container")
          .siblings(".vce-classic-tabs-panels-container")
    ...
    $("body").on("click", ".vce-classic-accordion-panel-heading", function () {
      var $this,
        $container,
        $wcpt = $(this).closest(".vce-classic-accordion-panel").find(".wcpt");
      setTimeout(function () {
    ...
    $("body").on("click", ".vc_tta-panel-heading", function () {
      var $this,
        $container,
        $wcpt = $(this).closest(".vc_tta-panel").find(".wcpt");
      setTimeout(function () {
        $wcpt.trigger("wcpt_layout", { source: "visual_composer__accordion" });
      }, 1);
    }),
    $("body").on("click", ".wpsm_panel-heading", function () {
      var $this,
        $container,
        $content = $(this).closest(".wpsm_panel").find(".wpsm_panel-collapse");
    ...
    $("body").on("click", ".kc_accordion_header", function () {
      var $this,
        $container,
        $content = $(this)
          .closest(".kc_accordion_section")
          .find(".kc_accordion_content");
    ...
    $("body").on("mouseup touchend", ".accordion__header", function () {
      var $this,
        $container,
        $content = $(this).closest(".accordion__item").find(".accordion__body");
    ...
    $("body").on("click", ".wpsm_nav li", function () {
      ...
      $this
        ...
        .eq(index)
        .find(".wcpt")
        .trigger("wcpt_layout", { source: "tabs_wpshopmart__tab" });
    }),
    ...
    function wcpt_ti_wishlist_growler(data) {
      ...
      $growler.attr("data-wcpt-icon", data.icon),
        ...
        data.url
          ...
          : wcpt_ti_wishlist_url && $growler.attr("href", wcpt_ti_wishlist_url),
        $("body").append($growler),
        setTimeout(function () {
    ...
    function wcpt_ti_wishlist_update_row_view($row) {
      ...
      if ("variable" !== product_type || variable_permitted || variation_id) {
        $buttons.removeClass("wcpt-disabled wcpt-loading");
        var list_id = variation_id || product_id;
        -1 === wcpt_ti_wishlist_ids.indexOf(list_id + "")
          ? $buttons.removeClass("wcpt-active")
          : $buttons.addClass("wcpt-active");
      } else $buttons.removeClass("wcpt-active").addClass("wcpt-disabled");
    }
    setTimeout(function () {
          ...
          if (-1 === wcpt_ti_wishlist_ids.indexOf(list_id))
            $.ajax({
              ...
              "variable" === product_type &&
                ...
                response &&
                  void 0 !== response.wishlists_data &&
            ...
            if (
              (index > -1 && wcpt_ti_wishlist_ids.splice(index, 1),
              wcpt_ti_wishlist_update_row_view($row),
              $wcpt.data("wcpt_sc_attrs").ti_wishlist)
            ) {
              var $remove_row = $row;
  ...
  function afrfqbt_status($container) {
    var $raq_buttons;
    void 0 === $container && ($container = $("body")),
      ($container.is(".wcpt-afrfqbt")
        ? $container
        : $(".wcpt-afrfqbt", $container)
      ).each(function () {
        var $raq_button = $(this),
        ...
        (id =
          "variable" === product_type
            ...
            ? $row.attr("data-wcpt-variation-id")
            : $row.attr("data-wcpt-product-id")),
          -1 !== $.inArray(parseInt(id), wcpt_afrfq_params.product_ids)
            ? ($raq_button.addClass("added"),
              $prev_view_link.length ||
                $raq_button.after(
            ...
            : ($raq_button.removeClass("added"), $prev_view_link.remove());
      });
  }
  $("body").on("select_variation", ".wcpt-row", function () {
    var $this = $(this),
      ...
      $wholesale_shortcode;
    $(".wcpt-wholesale", $this).each(function () {
      var $this = $(this);
      if (
        ($this.removeClass(
      ...
      if ($this.hasClass("wcpt-wholesale--original-price")) {
        ...
        if (variation) {
          var original_price = "";
          variation_text = format_price(
            (original_price = variation.wholesale_price
              ? variation.original_display_price
              : variation.display_price)
          );
        }
        $(".wcpt-wholesale__variation-view", $this).text(variation_text);
      }
      $this.hasClass("wcpt-wholesale--wholesale-label") &&
        ($this.removeClass(
    ...
    $("body").on(
      ...
      function () {
        ...
        $form.hasClass("wvs-loaded") &&
          get_select_variation_from_cart_form($form);
      }
    ),
    "undefined" == typeof wcpt_afrfq_params &&
      (wcpt_afrfq_params = {
        product_ids: [],
        view_quote_url: "",
        view_quote_label: "",
    ...
    $("body").on("wcpt_after_every_load", ".wcpt", function () {
      var $wcpt = $(this);
      $(".afrfqbt, .afrfqbt_single_page", $wcpt)
        .removeClass("afrfqbt afrfqbt_single_page")
        .addClass("wcpt-afrfqbt"),
    ...
    $("body").on("click", ".wcpt-afrfqbt", function (e) {
      ...
      var $this = $(this),
        ...
        qty = 0;
      if (
        ((qty = $(".cart .qty", $row).length
          ? $(".cart .qty", $row).val()
          : $(".qty", $row).val()) || (qty = 1),
        $row.hasClass("wcpt-product-type-variable")
          ? ((variation_id = $row.data("wcpt_variation_id")),
            (variation_attributes = $row.data("wcpt_attributes")))
          : $row.hasClass("wcpt-product-type-variation") &&
            ((variation_id = $row.attr("data-wcpt-variation-id")),
            (variation_attributes = $row.data("wcptVariationAttributes"))),
        !$row.hasClass("wcpt-product-type-variable") || variation_id)
      ) {
        if (!$this.hasClass("disabled")) {
          var ajax_data;
          if (($this.removeClass("added"), variation_id))
            ((ajax_data = {
            ...
            }).form_data = $.param(ajax_data.form_data)),
    ...
    $(".wcpt, .wcpt-lazy-load").length &&
      $(".wcpt").each(function () {
        after_every_load($(this));
      });
  var cart_init_required = !1;
  (-1 !== document.cookie.indexOf("woocommerce_items_in_cart") ||
    ("undefined" != typeof wcpt_cart_result_cache &&
      wcpt_cart_result_cache.cart_quantity)) &&
    (cart_init_required = !0),
    cart_init_required && wcpt_cart({ payload: { skip_cart_triggers: !0 } }),
    $(window).on("pageshow", function (e) {
      e.originalEvent.persisted &&
        wcpt_cart({ payload: { skip_cart_triggers: !0 } });
    });
}),
  jQuery(function ($) {
    ...
    $(".wcpt").each(store_parent_in_child_row_data),
      $("body").on(
        ...
        ".wcpt",
        store_parent_in_child_row_data
      ),
      $("body").on("click", ".wcpt-child-row-toggle", function () {
        ...
        $this.toggleClass("wcpt-child-row-toggle--closed"),
          $this.is("td")
            ...
            : $this.hasClass("wcpt-child-row-toggle--closed")
        ...
        var freeze_table = $(".wcpt-table.frzTbl-table", $wcpt).data(
          "freezeTable"
        );
        freeze_table && freeze_table.cell_resize();
      }),
      $("body").on(
        "click",
        ...
        function (e) {
          ...
          ).length || $(".wcpt-child-row-toggle", $row).click();
        }
      );
  }),
  jQuery(function ($) {
    function init_instant_search() {
      var $this = $(this),
        sc_attrs;
      $this.data("wcpt_sc_attrs").instant_search &&
        ($(".wcpt-search-wrapper", $this).addClass("wcpt-instant-search"),
        $(".wcpt-instant-search .wcpt-search-input", $this).on(
          "keydown",
          function (e) {
            (13 !== e.keyCode && 13 !== e.which) || e.stopPropagation();
          }
        ));
    }
  ...
  jQuery(function ($) {
    function init_instant_sort() {
      var $this = $(this),
        sc_attrs;
      if ($this.data("wcpt_sc_attrs").instant_sort) {
        var sort_data = [];
        $(".wcpt-row:not(.wcpt-child-row)", $this).each(function () {
          var $this = $(this),
            ...
            product_sort_data = $.extend(
              {},
              { id: id, variation_id: variation_id },
          ...
          $this.on("click.wcpt", ".wcpt-heading.wcpt-sortable", function () {
            ...
            $.each(all_sort_params.column_heading, function (id, params) {
              if ($sorting_icons.hasClass("wcpt-" + id))
                return (current_sort_params = params), !1;
            }),
    ...
    function instant_sort_ui_feedback(current_sort_params, $wcpt) {
      ...
      -1 !== $.inArray(current_sort_params.orderby, ["rating", "price-desc"]) &&
        ...
        "price-desc" == current_sort_params.orderby &&
        ...
        $.each(
          ...
          function (dropdown_option_index, option_sort_params) {
            if (
              ...
              "price-desc" == option_sort_params.orderby &&
              ...
              !(
                ...
                ("meta_key" == current_sort_params.orderby &&
                ...
                ("taxonomy" == current_sort_params.orderby &&
              ...
              var $dropdown = $('[data-wcpt-filter="sort_by"]', $wcpt),
                ...
                $heading_label;
              $(".wcpt-dropdown-label", $dropdown).text(
                $("span", $selected_option).text()
              ),
                $selected_input.prop("checked", !0),
                $selected_option
    ...
    function wcpt_assign_even_odd_row_classes($table) {
      ...
      $rows.each(function () {
        ...
        $child_rows.each(function () {
          var $this = $(this),
            $parent_row;
          $this.data("wcpt_parent_row").hasClass("wcpt-even")
    ...
    $(".wcpt").each(init_instant_sort),
      ...
      (window.wcpt_instant_sort = function (params, $wcpt) {
        ...
        switch (
          ...
          -1 !== $.inArray(params.orderby, ["price-desc", "rating"]) &&
            (params.order = "desc"),
          params.orderby)
        ) {
          case "title":
            sort_data.sort(function (a, b) {
              return "asc" == params.order
          ...
          case "price-desc":
            sort_data.sort(function (a, b) {
              var a_price = a.price,
                b_price = b.price;
              return (
                "asc" == params.order && a.min_price && (a_price = a.min_price),
                "desc" == params.order &&
                  a.max_price &&
                  (a_price = a.max_price),
                "asc" == params.order && b.min_price && (b_price = b.min_price),
                "desc" == params.order &&
                  b.max_price &&
                  (b_price = b.max_price),
                "asc" == params.order ? a_price - b_price : b_price - a_price
              );
            });
            break;
          case "meta_value":
          ...
          case "meta_value_num":
            sort_data.sort(function (a, b) {
              var a_meta = a["meta_value__" + params.meta_key],
                b_meta = b["meta_value__" + params.meta_key],
                a_meta_num = isNaN(parseFloat(a_meta)) ? 0 : parseFloat(a_meta),
          ...
          case "attribute_num":
            sort_data.sort(function (a, b) {
              var a_val = a["attribute__" + params.orderby_attribute],
                b_val = b["attribute__" + params.orderby_attribute],
                a_val_num = isNaN(parseFloat(a_val)) ? 0 : parseFloat(a_val),
        ...
        $.each(sort_data, function (index, product_sort_data) {
          var $row = !1;
          ($row = product_sort_data.variation_id
            ? $wcpt.find(
  ...
  jQuery(function ($) {
    function build_csv_and_download(json_data, headings, file_name) {
      ...
      $pseudo_link.appendTo("body").get(0).click(), $pseudo_link.remove();
    }
